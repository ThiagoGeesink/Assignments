...
\newline \newline A:
The data only records the curtate lifetime $K_i = \lfloor X_i \rfloor$, the integer part of the true age-at-death. A person recorded as dying at age $x$ actually died somewhere in the interval $[x,x+1)$. 
Assuming deaths are roughly uniformly distributed within each year of age, the best estimate of the actual age-at-death for such a person is the midpoint $x+0.5$. 
Using just $x$ would systematically underestimate all ages-at-death by about half a year.
...
\newline \newline A: 
The Makeham model was fitted ignoring the ages $x=0,1,2$, because those early deaths are driven by infant mortality rather than the Makeham mechanism. 
The Makeham distribution therefore does not account for the infant deaths that pull the sample mean down. The Makeham estimate of $\mathbb{E}[X]$ effectively models the lifetime of someone not subject to infant mortality, and thus higher than the raw sample average.
...
[1] 8.898422
\end{verbatim}
Since $S_X(x)$ is larger under the Makeham model than the true empirical survival function (which is pulled down by infant deaths), the integral $\int_{65}^\infty S_X(x)\,dx$ is also larger, yielding a higher estimated expected pension duration.
...
\newline \newline A:
When computing $\mathbb{E}[(X-65)_+]$ for the whole population, the comparison includes everyone.
The Makeham model ignores those early deaths and thus overestimates the probability of surviving to old age for the full cohort, causing it to give a higher value than the sample.
However, when we condition on $X>65$, we restrict attention to people who actually survived past 65. 
At this point, the infant deaths are already "filtered" out of both the sample and the model.
...
Starting from equation (3), the original survival function satisfies:
\[S_X(x) = \exp\!\left(-\int_0^x \mu_X(t)\,dt\right)\]
With the new mortality rate \[ \mu_X^{\text{new}}(x) = f \cdot \mu_X(x)\], 
the new survival function is:
\[S_X^{\text{new}}(x) = \exp\!\left(-\int_0^x f\cdot\mu_X(t)\,dt\right) = \exp\!\left(-f\int_0^x \mu_X(t)\,dt\right) = \left[S_X(x)\right]^f\]
So multiplying the mortality rate by a factor $f$ simply raises the survival function to the power $f$.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{Q8b.png}
\caption{Expected age-at-death as a function of $k$, where $f = 2^k$.}
\end{figure}
This drop is approximately 6 years per doubling of the mortality rate.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{Q9b.png}
\caption{Observed fractions of deaths at each age, together with fitted Makeham and Gompertz densities.}
\end{figure}
From the plot, it appears that the Makeham density (red line) fits the data better than the Gompertz density (blue dashed line), especially at younger ages.
