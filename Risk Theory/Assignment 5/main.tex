...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    A: 
    From the \texttt{Next} matrix, column 7 gives the destination classes: 0 claims $\to$ class 8, 1 claim $\to$ class 4, and both 2 and 3+ claims $\to$ class 1. Accumulating probabilities over all transitions that lead to each destination:
    \begin{align*}
      P_{7,1} &= \Pr[\text{2 claims}] + \Pr[\text{3+}] = 0.10 + 0.05 = 0.15, \\
      P_{7,4} &= \Pr[\text{1 claim}] = 0.20, \\
      P_{7,8} &= \Pr[\text{0 claims}] = 0.65, \\
      P_{7,j} &= 0 \quad \text{for all other } j.
    \end{align*}
...
\end{lstlisting}
      All 14 column standard deviations lie between $10^{-20}$ and $10^{-17}$, so the chain has converged to its steady-state distribution after 1024 steps regardless of the starting class.
...
\end{lstlisting}
The deviation of each row sum from 1 is on the order of $10^{-14}$. Every row therefore sums to exactly 1, confirming that $\mathbf{P}^{1024}$ remains a proper probability matrix after repeated squaring.
    \end{enumerate}
\end{enumerate}
\clearpage
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    A:
    Let $\lambda^{\pm} = \lambda(1 \pm \varepsilon)$
    \begin{align*}
      &\log\!\bigl(0.998\,b(\lambda^+)\bigr) - \log\!\bigl(1.002\,b(\lambda^-)\bigr) \\
      &\quad= \bigl[\log b(\lambda^+) + \log 0.998\bigr] - \bigl[\log b(\lambda^-) + \log 1.002\bigr].
    \end{align*}
    Applying $\log(1+x) \approx x$ for $|x|$ small: $\log 0.998 = \log(1-0.002) \approx -0.002$ and $\log 1.002 \approx 0.002$, so the numerator becomes $[\log b(\lambda^+) - \log b(\lambda^-)] - 0.004$. For the denominator, $\log(\lambda^+) - \log(\lambda^-) = \log\!\frac{1+\varepsilon}{1-\varepsilon} \approx 2\varepsilon$. Dividing gives
    \[
      \frac{[\log b(\lambda^+) - \log b(\lambda^-)] - 0.004}{2\varepsilon}
      = \underbrace{\frac{\log b(\lambda^+) - \log b(\lambda^-)}{\log \lambda^+ - \log \lambda^-}}_{\approx\, e(\lambda)} - \frac{0.004}{2\varepsilon}
      = e(\lambda) - \frac{1}{500\varepsilon}.
    \] \clearpage
...
[1] 0.10304
\end{lstlisting}
The five simulation estimates range from roughly 4 to 19, all far from the exact value of $0.103$. This is exactly the noise predicted in part (a): with $\varepsilon = 10^{-4}$.
...
[1] 0.10304
\end{lstlisting}
With $\varepsilon = 0.1$, all five estimates fall within the range $[0.092, 0.116]$, close to the exact value of $0.103$. The noise bias is now $1/(500 \times 0.1) = 0.02$, which is of similar magnitude to the random error and small enough to yield useful estimates but at the cost of a larger approximation error from using a bigger difference.
  \end{enumerate}
\end{enumerate}
...
[1] 3.254
\end{lstlisting}
A reduction of approximately $3.3\%$
\end{enumerate}
\clearpage
...
[1] 7.45
\end{lstlisting}
The extended BM system reduces the steady-state premium from $0.3261$ to $0.3018$, a decrease of approximately $7.5\%$. The bonus guarantee is more generous than the leap-year ($3.3\%$).
  \end{enumerate}
\end{enumerate}
\clearpage
...
\end{lstlisting} \clearpage
    ...
    \end{center}
    The ratio is lowest ($\approx 0.816$) around $\lambda = 0.15$. Drivers with an intermediate claim frequency (about 1 claim every 6-7 years) benefit most
